 ...
 * Authority,
 * and send everything to the client side: there would be no View Model and DTO, a lot less code,
 * and an outer-join
 ...
 * the time for nothing (for performance reasons). This is the #1 goal: we should not impact our
 * users'
...
public class UserResource {
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the "UUID" user, or with status
     *         404 (Not Found)
     */
    ...
    public ResponseEntity<UserDTO> getUserByUuid(@PathVariable String uuid) {
        ...
        return ResponseUtil.wrapOrNotFound(userService.findByUuid(uuid).map(UserDTO::new));
    }
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable String uuid) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createAlert("userManagement.deleted", uuid))
                .build();
    }
